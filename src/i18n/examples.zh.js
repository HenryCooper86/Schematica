...
export default {
  ...
  'ev-bms': {
    ...
    title: '电动汽车电池管理',
    nodes: {
      n1: { label: '高压电池包', notes: '96 节电芯串联；只要 BMS 断开接触器，电池包就与整车隔离。' },
      n2: { label: '烟火保险丝' },
      n3: { label: '主接触器' },
    ...
    journey: {
      j1: { label: '高压电池包', caption: '96 节电芯、一个烟火保险丝和两个接触器：没有 BMS 的允许，什么都出不了电池包。' },
    ...
    wires: { w3: '预充', w9: '菊花链', w18: '状态', w21: '电芯采样线', w23: '电芯采样线', w29: '线圈回线', w30: '线圈回线' },
  },
  'journey-adas': {
  ...
  'hsd600-adas': {
    ...
    notes: { t1: 'HSD 600 传感器配置：11 路摄像头绘制为 4 组，3 个雷达；可选激光雷达未绘制。摄像头、雷达和交换机的供电未绘制。' },
    journey: {
      j1: { label: '11 路摄像头', caption: '11 路 GMSL2 摄像头以 4 组接入控制器：前向 3 路、每侧 2 路、后视与泊车 4 路。线束中每个摄像头各有自己的同轴线；分组只是为了让图面易读。' },
  ...
  'rdk-x3-robot': {
    ...
    journey: {
      ...
      j3: { label: '经 UART 的运动控制', caption: 'X3 没有 CAN FD，因此由基于 Cortex-M4 的驱动微控制器通过 UART 接收速度指令并以 PWM 驱动两个轮子。驱动电路和回路需另行验证。' },
  ...
  'ota-security': {
    ...
    nodes: {
      ...
      t2: { label: '中间人攻击者', notes: '试图在 CDN 与车辆之间替换镜像。' },
    ...
    journey: {
      ...
      j3: { label: '威胁', caption: '恶意工程师、中间人攻击者和木马镜像各针对一个环节；签名校验将三者一并挫败。' },
  ...
  'secure-boot': {
    ...
    journey: {
      j5: { label: '引导程序被拒绝', caption: '引导程序签名验证失败后，沿明确的停机分支执行。' },
      j6: { label: '恢复槽位', caption: '应用检查失败后选择另一个槽位并返回验证。' },
