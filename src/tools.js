...
export function createTools({ svg, store, requestRender, onToolChange, onSave, library }) {
  ...
  const { copySelection, pasteClipboard } = createClipboardCommands({ store, library, notify: toast,
    canEditNow: () => !ui.busy && !store.inBatch() && !document.querySelector('dialog[open]'),
  });
