...
<nav>${['board', 'findings', 'interfaces', 'requirements', 'decisions', 'power', 'bom', 'verification', 'comments', ...(report.comparison ? ['changes', 'impact'] : [])].map((id) => `<a href="#${id}">${id[0].toUpperCase() + id.slice(1)}</a>`).join('')}</nav>
