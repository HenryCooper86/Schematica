...
export function renderReviewHTML(report, files, viewers) {
  ...
  sections.push(
    `<section id="power"><h2>Power budgets</h2><p>Operating mode: <strong>${esc(report.assumptions.mode)}</strong>. Peak scenario: <strong>${esc(report.assumptions.peaks)}</strong>.</p>${table(
    ...
    )}<p>${esc(report.assumptions.note)}</p></section>`,
  );
  sections.push(
...
<nav>${['board', 'findings', 'interfaces', 'requirements', 'decisions', 'power', 'bom', ...(report.comparison ? ['changes', 'impact'] : [])].map((id) => `<a href="#${id}">${id[0].toUpperCase() + id.slice(1)}</a>`).join('')}</nav>
<div class="summary"><span>${counts.error} errors · ${counts.warning} warnings · ${counts.info} information items</span><span>${report.coverage.allocated}/${report.coverage.total} requirements fully allocated</span><span>${report.coverage.verifiedWithEvidence}/${report.coverage.total} verified with evidence and valid allocations</span></div></header>
...
${sections.join('')}<section id="downloads"><h2>Download source artifacts</h2>${Object.keys(files)
    ...
    .join('')}</section></main>
<script type="application/json" id="review-data">${scriptJSON({ files, viewers })}</script><script>
const {files,viewers}=JSON.parse(document.getElementById('review-data').textContent),frame=document.querySelector('iframe'),scope=document.getElementById('scope');let pending=[];
const focus=()=>frame.contentWindow.postMessage({type:'schematica-review-focus',ids:pending},'*');frame.addEventListener('load',focus);
...
frame.dataset.scope='[]';scope.onchange=()=>show(scope.value,[]);document.querySelectorAll('[data-scope]').forEach(b=>b.onclick=()=>{show(b.dataset.scope,JSON.parse(b.dataset.ids));document.getElementById('board').scrollIntoView();});
