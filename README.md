...
**Check → Review readiness** reports interface coverage separately from design findings:
checked, failed, unassessed, and not applicable. Missing declarations never count as
checked. Flow/relationship links are excluded; power/ground require direction,
voltage ranges, and sources rather than protocol/rate fields. Checks compare
supplied declarations, not physical hardware or the authenticity of sources.

Use `npm run check:board -- --review-ready --json board.schematica.json` to require
at least one applicable connection, complete compatible declarations for all
applicable connections, and no design errors or warnings. `--strict` retains its
existing warning policy; JSON output always includes coverage and `reviewReady`.
The **Declared UART Interface Reference** example demonstrates a complete,
illustrative contract. Its assumptions are not vendor specifications. Review
packages include the same coverage and readiness result, including subsystems.
